...
final class bot {
	...
	private function createBot($bot)
	{
		...
		$bot->parserClass->setDatabase2($bot->db2);
	...
	private function connectToDatabase($bot)
	{
		if (isset($bot->config['usedatabase']))
		{
			...
			//Connect to additional pastes DB
			$bot->db2 = new $dbType($bot->config['dbhost2'],
								$bot->config['db2'],
								$bot->config['dbuser2'],
								$bot->config['dbpass2'],
								$bot->config['dbprefix'],
			...
			if (!$bot->db2->isConnected())
			{
				die("Couldn't connect to database2...");
			}
